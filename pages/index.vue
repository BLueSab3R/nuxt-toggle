    ...
    <div class='flex flex-col items-center justify-center h-screen'>
        ...
        <ToggleButton @turn-on="turnOn" @turn-off='turnOff' :accept="accept">
            ...
            <button type="button" :class="{
                ...
                'relative border-2 border-transparent transition-colors  duration-200 ease-in-out': type === 'icon',
            }"
                ...
                <span v-if="type === 'simple' || type === 'short'" aria-hidden="true" :class="{
                    'translate-x-0': !accept,
                    'translate-x-5': accept,
                    'inline-block h-5 w-5 transform bg-white shadow ring-0 transition': type === 'simple',
                    'absolute  inline-block h-5 w-5 transform rounded-full border border-gray-200 bg-white shadow ring-0 transition-transform duration-200 ease-in-out': type === 'short'
                }" class='pointer-events-none rounded-full transition-colors duration-200 ease-in-out'>
                </span>
                <span v-if="type === 'icon'" :class="{
                    'translate-x-0': !accept,
                    'translate-x-5': accept,
